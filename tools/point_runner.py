...
class PointPipRunner:
    def __init__(self, model_config = "PoinTr_doge/cfgs/PCN_models/PoinTr.yaml", model_checkpoint = "PoinTr_doge/ckpts/PoinTr_PCN_FT.pth", device='cuda:0'):
        self.model_config = model_config
        self.model_checkpoint = model_checkpoint
        self.device = device

    def inference_single(self, model, pcd, device, config):
    ...
    def point_pip_runner(self, pcd):
        # init config
        config = cfg_from_yaml_file(self.model_config)
        # build model
        base_model = builder.model_builder(config.model)
        builder.load_model(base_model, self.model_checkpoint)
        base_model.to(self.device.lower())
        base_model.eval()

        complete_point = self.inference_single(base_model, pcd, self.device, config)
